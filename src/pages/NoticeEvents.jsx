...
function CategoryBadge({ cat }) {
  const colors = {
    Academic: 'bg-blue-50 text-blue-600', Library: 'bg-green-50 text-green-600', Facility: 'bg-orange-50 text-orange-600',
    Events: 'bg-purple-50 text-purple-600', Finance: 'bg-red-50 text-red-600', Tech: 'bg-cyan-50 text-cyan-600',
    Cultural: 'bg-pink-50 text-pink-600', Sports: 'bg-emerald-50 text-emerald-600',
  }
  return <span className={`px-2 py-0.5 rounded font-semibold text-[10px] ${colors[cat] || 'bg-gray-50 text-gray-600'}`}>{cat}</span>
}
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 ml-0 md:ml-64 flex flex-col min-h-screen">
        <header className="sticky top-0 z-30 flex items-center gap-4 px-6 md:px-8 py-4 bg-white/85 backdrop-blur-lg border-b border-gray-200">
          ...
          <h1 className="text-lg font-bold text-gray-900">{tab === 'notices' ? 'Notices & Announcements' : 'Campus Events'}</h1>
        </header>
        ...
        <main className="flex-1 p-6 md:p-8 max-w-6xl w-full">
          ...
          <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{tab === 'notices' ? 'Notices' : 'Events'}</h1>
              <p className="text-sm text-gray-500 mt-1">{tab === 'notices' ? 'Stay updated with all campus announcements.' : 'Discover and register for campus events.'}</p>
            </div>
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-2 bg-gray-100 rounded-xl px-3.5">
                <Search size={16} className="text-gray-400 shrink-0" />
                <input type="text" placeholder={`Search ${tab}...`} value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} className="flex-1 bg-transparent border-none outline-none py-2.5 text-sm text-gray-900 placeholder:text-gray-400 w-40" />
              </div>
              <select value={filter} onChange={e => setFilter(e.target.value)} className="bg-gray-100 border-none rounded-xl px-3.5 py-2.5 text-sm text-gray-700 font-medium outline-none cursor-pointer">
          ...
          <div className="flex gap-1 mb-6 bg-gray-100 p-1 rounded-xl w-fit">
            {['notices', 'events'].map(t => (
              <button key={t} onClick={() => handleTabChange(t)} className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all cursor-pointer ${tab === t ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                ...
                <>
                  <div className="grid md:grid-cols-2 gap-4">
                    {paginatedNotices.map(n => (
                      <div key={n.id} className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm hover:shadow-md transition-all">
                        ...
                        <p className="text-sm text-gray-500 leading-relaxed mb-4">{n.description}</p>
                        <div className="flex items-center gap-3">
                          <button className="flex items-center gap-1.5 text-xs font-semibold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-all cursor-pointer"><ArrowRight size={14} /> Read More</button>
                        </div>
                      </div>
                    ))}
                  </div>
                  {totalPages > 1 && (
                    <div className="flex items-center justify-center gap-3 mt-8">
                      <button disabled={page === 1} onClick={() => setPage(p => Math.max(1, p - 1))} className="flex items-center gap-1.5 text-sm font-medium text-gray-600 px-4 py-2 rounded-xl border border-gray-200 hover:bg-gray-50 transition-all disabled:opacity-40 cursor-pointer"><ChevronLeft size={16} /> Previous</button>
                      <div className="flex gap-1.5">
                        {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
                          <button key={n} onClick={() => setPage(n)} className={`w-9 h-9 rounded-xl text-sm font-semibold transition-all cursor-pointer ${page === n ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'}`}>{n}</button>
                        ))}
                      </div>
                      <button disabled={page === totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))} className="flex items-center gap-1.5 text-sm font-medium text-gray-600 px-4 py-2 rounded-xl border border-gray-200 hover:bg-gray-50 transition-all disabled:opacity-40 cursor-pointer">Next <ChevronRight size={16} /></button>
                    </div>
                  )}
                </>
              )}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-all">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-500 mb-2.5"><MapPin size={13} /> {e.venue}</div>
        <h3 className="font-bold text-gray-900 mb-1.5">{e.title}</h3>
        <p className="text-xs text-gray-500 leading-relaxed mb-4">{e.description}</p>
        <button onClick={() => setRegistered(true)} disabled={registered} className={`w-full flex items-center justify-center gap-2 text-sm font-semibold py-2.5 rounded-xl transition-all cursor-pointer ${registered ? 'bg-green-50 text-green-600' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
